...
def train_one(csv_train, dir_train, csv_val, dir_val, args):
    ...
    w = w * scale
    w = w / (w.mean() + 1e-6)
    crit = nn.CrossEntropyLoss(weight=torch.tensor(w, dtype=torch.float32, device=device), label_smoothing=0.10) 
...
if __name__ == "__main__":
    ...
    ap.add_argument("--es-patience", type=int, default=10, help="early stopping patience based on val_F1")
    # dataset paths
